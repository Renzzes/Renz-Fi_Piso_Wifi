...
import { useVouchers, useGenerateVouchers } from "@/hooks/api/useVouchers";
...
export default function VouchersPage() {
  ...
  const filtered = vouchers.filter(
    (v) =>
      (filter === "all" || v.status === filter) && v.code.toLowerCase().includes(q.toLowerCase()),
  );

  const generate = (count: number, amount: number, minutes: number) => {
    generateMutation.mutate({ count, amount, minutes }, { onSuccess: () => setOpen(false) });
  };
