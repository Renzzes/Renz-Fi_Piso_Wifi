...
const HEALTH_STYLE: Record<StorageHealth, string> = {
  HEALTHY: "border-emerald-300 bg-emerald-50 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300",
  DEGRADED: "border-orange-400 bg-orange-50 text-orange-800 dark:border-orange-800 dark:bg-orange-950/30 dark:text-orange-300",
  WARNING: "border-yellow-400 bg-yellow-50 text-yellow-900 dark:border-yellow-700 dark:bg-yellow-950/30 dark:text-yellow-300",
  CRITICAL: "border-red-400 bg-red-50 text-red-800 dark:border-red-800 dark:bg-red-950/30 dark:text-red-300",
  READ_ONLY: "border-orange-400 bg-orange-50 text-orange-800 dark:border-orange-800 dark:bg-orange-950/30 dark:text-orange-300",
...
const HEALTH_COPY: Record<
  StorageHealth,
  { status: string; description: string; action: string }
> = {
  HEALTHY: {
    status: "Healthy",
...
function notifyTransitionExtras(
  previous: StorageStatus | null,
  next: StorageStatus,
) {
...
export function StorageHealthCard({ className }: { className?: string }) {
  const { data, isLoading, isError } = useStorageHealth();
  ...
  const rows = [
    ...
    ["Readable / writable", data ? `${data.readable ? "Yes" : "No"} / ${data.writable ? "Yes" : "No"}` : "Unknown"],
    ...
    ["Pending history replay", data?.pendingReplay === undefined ? "Unknown" : String(data.pendingReplay)],
    ["Pending conflicts", data?.pendingConflicts === undefined ? "Unknown" : String(data.pendingConflicts)],
    [
      "Reconciliation",
    ...
    ["Retry remaining", data?.retryRemaining === undefined ? "Unknown" : String(data.retryRemaining)],
    ["Watch mode", data?.watchMode === undefined ? "Unknown" : data.watchMode ? "Yes" : "No"],
    ["Recovery mode", data?.recoveryMode === undefined ? "Unknown" : data.recoveryMode ? "Yes" : "No"],
  ...
  return (
    <section className={cn("rounded-md border bg-card p-3", className)} aria-label="SD Card Health">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={cn("rounded border px-2 py-0.5 text-xs font-semibold", HEALTH_STYLE[health])}>
          {isLoading ? "Loading" : isError ? "Unknown" : copy.status}
        </span>
      </div>
      ...
      {data?.conflicts?.length ? (
        <div className="mt-3 space-y-1 rounded-md border border-orange-300 bg-orange-50 p-2 text-xs text-orange-900 dark:border-orange-800 dark:bg-orange-950/30 dark:text-orange-300">
          <p className="font-medium flex items-start gap-1.5">
            <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
            SPIFFS vs SD copy differs — owner review required (no automatic merge).
            SD may still be mounted and working.
          </p>
